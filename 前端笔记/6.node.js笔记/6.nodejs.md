...
```js
//3.secret密钥的本质就是一个字符串
const secretKey='QR-KK NO.1'
```
...
status:200,
message:'登陆成功',
//调用jwt.sign()生成JWT字符串，三个参数分别为用户信息对象、加密密钥、配置对象
token:jwt.sign({username:userinfo.username},secertkey,{expiresIn:'30s'})
})
})
```
...
客户端每次在访问那些有权限接口的时候，都需要主动通过请求脱的Authorization字段，将Token字符串发送给服务器进行身份认证
...
当express-jwt这个中间件配置成功之后，即可在哪些有权限的接口中使用req.user对象，来访问JWI字符串中解析出来的用户信息了，示例代码如下：
...
当使用express-jwt解析Token字符串时，如果客户端发送过来的Token字符串国旗或不合法，会产生一个解析失败的错误，影响项目的正常运行，我们可以通过Express的错误中间件，捕获这个错误并进行相关处理，示例代码如下：
